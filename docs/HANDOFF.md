...
```text
创建任务（下拉选择已有医生）或导入任务（按手机号创建/匹配医生账号）
  → 查看任务及审核进度
  → 查看逐条审核证据
  → 查看医生提现申请
  → 导出基金会线下处理名单
...
| 任务管理 | 手动创建任务（下拉选择已有医生，禁用医生不可选）；CSV 名单校验预览（模板含导入日期，同一医生同一导入日期重复导入拦截，日期不同可再次导入）；导入时按手机号创建或复用医生账号；整批创建任务；禁用账号拦截；列表、筛选、详情（批次展示名 `display_title`＝「药品知识库审核 · 批次号」）、进度和积分 |
...
- 积分口径：内部字段一律 `*_cent`（分），前后台展示统一为积分（1 积分 = 1 元，50 积分/条），后台页面不出现 ¥；仅基金会提现导出 CSV 保留「金额(元)」列。提现到账口径为人工审核后 7–15 个工作日。
...
| 已申请提现 | 12,500 积分 |
| 可提现 | 2,500 积分 |
| 待导出提现 | 8 笔，共 5,000 积分 |
| 已导出提现（待结算） | 2 笔，共 4,500 积分 |
| 已结算提现 | 1 笔，共 3,000 积分 |
